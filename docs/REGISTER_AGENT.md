...
```
plugin.yaml 或 opencode.json MCP  → 注册 sdk-forge MCP 工具
...
> **注意**：SDK Forge 是 Python MCP 插件，**不会**出现在 OpenCode 的 npm「插件」列表中。请在 MCP 列表中查找 `sdk-forge`，在 Agent 下拉框中查找 `forge`。
...
```json
{
  "mcp": {
    "sdk-forge": {
      "command": ["python", "C:/path/to/sdk-forge/mcp_server.py"],
      "enabled": true,
      "type": "local"
    }
  }
...
```yaml
# plugin.yaml
name: sdk-forge                          # 插件名称（唯一）
description: SDK Forge — auto-generate... # 插件描述
version: 2.5.0                                 # 版本号
...
# plugin.yaml
name: sdk-forge
description: SDK Forge — auto-generate GTest test suites from C/C++ SDK headers
version: 2.5.0
...
```json
{
  "command": {
    "forge": {
      ...
      "description": "SDK Forge — 生成 GTest 测试",
      "agent": "forge"
    }
  }
}

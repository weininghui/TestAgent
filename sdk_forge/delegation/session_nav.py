...
def _extract_message_text(message: dict[str, Any]) -> str:
    ...
    for part in message.get("parts") or []:
        ...
        if ptype in ("text", "reasoning") and part.get("text"):
            ...
            error = state.get("error") or part.get("error") or state.get("message") or ""
            label = f"[{tool_name}]"
            if title:
...
def export_session_preview_impl(session_id: str, max_chars: int = 500) -> dict[str, Any]:
    ...
    return _build_session_preview(data, session_id, max_chars)


def _build_session_preview(data: dict[str, Any], session_id: str, max_chars: int) -> dict[str, Any]:
    ...
    from sdk_forge.delegation.health import analyze_session_export

    health = analyze_session_export(data)
    return {
        "status": "ok",
    ...
    for entry in pending + completed:
        ...
        if include_preview and sid:
            ...
            if preview.get("status") == "ok":
                ...
                row["health"] = preview.get("health") or "ok"
                row["issues"] = preview.get("issues") or []
                if row["health"] not in ("ok", "pending"):
                    from sdk_forge.delegation.health import _recovery_hint

                    row["recovery"] = _recovery_hint(entry, str(row["health"]))

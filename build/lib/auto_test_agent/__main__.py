...
# Ensure the containing directory (site-packages when installed, or project root
# in editable mode) is on ``sys.path`` so ``agents.*`` and ``agent`` (installed
# via pyproject.toml py-modules) are importable.
...
def _add_run_parser(sub: argparse.ArgumentParser) -> None:
    ...
    sub.add_argument(
        "--output-root", "-o", default="./output", help="Output directory"
    )
...
def cmd_list_agents(args: argparse.Namespace) -> None:
    ...
    header = (
        f"{'Name':<16} {'Role':<16} {'Model':<28} {'Temp':<6} {'Prompt Stage':<16}"
    )
    sep = "-" * len(header)
...
def main() -> None:
    ...
    # run
    _add_run_parser(sub.add_parser("run", help="Run the agent with a goal"))

    # chat (interactive)
    _add_chat_parser(
        sub.add_parser("chat", help="Start interactive terminal session")
    )

    # list-agents
    _add_list_agents_parser(
        sub.add_parser("list-agents", help="List registered sub-agents")
    )

    # list-models
    _add_list_models_parser(
        sub.add_parser("list-models", help="Show available model presets")
    )

    # show-config
    _add_show_config_parser(
        sub.add_parser("show-config", help="Show full agent configuration")
    )
...
# Entry-point export (used by pyproject.toml [project.scripts])
_cli_main = main
